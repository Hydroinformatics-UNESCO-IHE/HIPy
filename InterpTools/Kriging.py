...
def Simple_Krig(SiteInfo, XYTargets, DataRecord):
    ...
    '''
    Loc, POIC, Prec = load_data(SiteInfo, XYTargets, DataRecord)
                      
    SVExp, CovMea = exp_semivariogram(Prec, Loc)
    
    xopt, ModOpt, VarFunArr = theor_variogram(SVExp)
    
    Z, SP, ZAvg = Krig(10, POIC, Loc, Prec, CovMea, ModOpt, xopt, 
                               VarFunArr)

    data_save_pkl(Z, 'PrecField')
    data_save_pkl(ZAvg, 'AvgPrec')
    data_save_pkl(SP, 'PrecUnc')
