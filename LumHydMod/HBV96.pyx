...
def Prec(double T,double LTT,double UTT,double P,double RFCF,double SFCF):
    '''
...
def Snow(double CFMAX,double TFAC,double T,double TTM,double CFR,double CWH,
    ...
    if T > TTM:
        ...
        if CFMAX*TFAC*(T-TTM)<SPOld+SF:
            MELT = CFMAX*TFAC*(T-TTM)
        else:
            MELT = SPOld+SF
        ...
        if CFR*CFMAX*TFAC*(TTM-T)<WCOld+RF: 
            REFR = CFR*CFMAX*TFAC*(TTM-T) 
        else:
            REFR = WCOld+RF 
...
def Resp(double TFAC,double PERC,double ALPHA,double K,double K1,double AREA,
    ...
    if TFAC*PERC < UZInt1: 
        LZInt1 = LZOld+TFAC*PERC
    else:
        LZInt1 = LZOld+UZInt1 


    if UZInt1 > TFAC*PERC: 
        UZInt2 = UZInt1-TFAC*PERC
    else:
        UZInt2 = 0.0
    ...
    UZNew = UZInt2-(Q0*TFAC) 
    LZNew = LZInt1-(Q1*TFAC)
...
def Run(p,p2,v,St):
    ...
    RF,SF = Prec(T,LTT,UTT,Pr,RFCF,SFCF)
